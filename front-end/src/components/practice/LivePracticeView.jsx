...
import { ArrowLeft, Timer, Camera, Bot, CheckCircle2 } from "lucide-react";
...
function LivePracticeContent() {
  ...
  const onHandResults = useCallback(
    (results) => {
      ...
      if (landmarks) {
        ...
        if (frameCountRef.current % PREDICT_EVERY_N === 0) {
          ...
          // Logic Tahan Posisi 2 Detik (Static Gestures)
          if (next.confidence >= 90) {
              ...
              if (progress >= 100) {
                if (lessonId) {
                  ...
                  fetch(`/api/lessons/${lessonId}/complete`, { method: "POST" })
                    ...
                    .finally(() => {
                      router.push(`/live-practice/summary?lessonId=${lessonId}`);
                    });
                } else {
                  router.push('/live-practice/summary');
                }
              }
            }
          } else {
    ...
    [handDetected, maybeSetResult, lessonId, router]
  );
  ...
  const setupMediaPipe = useCallback(async () => {
    ...
    try {
      ...
      const processFrame = async (now) => {
        if (window.isUnmountedLocal || !webcamRef.current || !webcamRef.current.video) {
          return;
        }
  ...
  const displayChar = conf >= MIN_CONF_SHOW && result.smoothedChar !== "..." ? result.smoothedChar : "";
          ...
          <div className="bg-white border border-gray-200 shadow-sm rounded-lg flex flex-col items-center justify-center relative overflow-hidden group p-6">
            ...
            <div className="z-10 flex flex-col items-center">
              ...
              <div className="w-44 h-44 bg-gray-50 rounded-lg shadow-sm flex items-center justify-center relative z-20 mb-4 border border-gray-200">
                <span className="text-6xl font-black text-blue-700">{lessonTitle.replace("Huruf ", "").replace("Angka ", "")}</span>
              </div>
